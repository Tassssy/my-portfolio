...
export function ResponsiveImage({ image, eager = false, className = '', sizes, fetchPriority, ...props }) {
  return <img className={className} {...imageProps(image, {eager,sizes,fetchPriority})} alt={image.alt} style={{objectPosition:image.position}} {...props}/>;
}
...
export function VideoPlayer({ video }) {
  ...
  const play = async () => { try { if (!player.current.getAttribute('src')) player.current.src = assetUrl(video.src); await player.current.play(); setStarted(true); } catch { setError(true); } };
  return <figure className="video-figure"><div className="video-heading"><span>VIDEO / {video.title}</span></div><div className="video-stage"><video ref={player} controls={started} playsInline preload="none" poster={started ? imageProps({src:video.poster}).src : undefined} aria-label={video.title} onError={()=>setError(true)} onPlay={event=>{setStarted(true);document.querySelectorAll('video').forEach(other=>{if(other!==event.currentTarget)other.pause()});}}>你的浏览器不支持在线播放，可打开视频文件。</video>{!started&&<button type="button" className="video-cover" onClick={play} aria-label={`播放视频：${video.title}`}><ResponsiveImage image={{src:video.poster,alt:`${video.title} 视频封面`}}/><span className="video-cover-play" aria-hidden="true">▶</span><span className="video-cover-label">点击播放 <span>{video.duration || 'WATCH VIDEO'}</span></span></button>}</div><figcaption><span>{video.caption}</span><a href={assetUrl(video.src)} target="_blank" rel="noopener noreferrer">{error ? '打开视频文件' : '独立窗口播放'} <Arrow diagonal/></a></figcaption></figure>;
}

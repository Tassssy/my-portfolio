...
export function ProjectArt({ project }) {
  return <div className={`project-art ${project.color}`} aria-hidden="true"><span className="art-index">FIELD NOTES / 0{project.number}</span>
    ...
    {project.cover && <div className="cover-evidence"><ResponsiveImage image={{...project.cover,alt:""}} sizes="(max-width:600px) 220px, 300px"/><span className="cover-evidence-label">{project.cover.label || "真实项目记录"} <Arrow diagonal/></span></div>}
